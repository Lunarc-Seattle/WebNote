...
- [📑 目录](#-目录)
  ...
  - [📌 总结记忆：](#-总结记忆)
    ...
    - [简写：](#简写)
      - [Reading the query params](#reading-the-query-params)
...
You must .send() a response, Or browser waits until timeout
You can't change response once you .send()
console.log() goes to server//console.log() 输出的内容会出现在服务器的控制台上，而不是浏览器的控制台。这是因为 console.log() 是在服务器端执行的。
